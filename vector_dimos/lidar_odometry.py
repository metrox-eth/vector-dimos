...
class LidarOdometry(Module):
    ...
    async def handle_pointcloud(self, msg: PointCloud2) -> None:
        ...
        if not self._searching:
            ...
            # The RealSense-only switch lives in costmap2d (LIDAR_WRITES_
            # OBSTACLES, env VECTOR_LIDAR_TO_MAP): the cloud stays FULL here -
            # the lidar points keep ticking the costmap's cadence and stay
            # z-tagged; whether they WRITE the world is the map's decision.
            # (First wiring 30/08 filtered the cloud HERE and starved the
            # whole pipeline: no obstacle ahead -> no publish -> no map.)
