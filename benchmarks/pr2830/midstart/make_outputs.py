...
COLUMNS = [
    ...
    # the two ping-pong indicators of the mid-start re-run
    ("goal_jump_total_m", "saut_but_a_but_total_m"),
    ("cross_map_swings", "traversees_de_carte"),
    ("swing_threshold_m", "seuil_traversee_m"),
    ("share_goals_beyond_5m_pct", "part_buts_au_dela_5m_pct"),
...
def main(argv=None):
    ...
    # Added for the big-office run: an optional output suffix, so the two bench
    # volumes can sit side by side. Nothing else changed.
    suffix = args[1] if len(args) > 1 else ""
    with open(path) as fh:
        data = json.load(fh)
    rows = [r["summary"] for r in data["results"]]
    write_csv(rows, os.path.join(HERE, f"resultats{suffix}.csv"))
    ...
    for config in ("shipped", "scoring"):
        ...
        for key, label, lib in [
            ...
            ("goal_jump_total_m", "saut but->but CUMULE (m) [ping-pong a]", True),
            ("cross_map_swings", "traversees de carte [ping-pong b]", True),
            ("share_goals_beyond_5m_pct", "part des buts au-dela de 5 m (%)", True),
        ...
        # the ping-pong pair, pooled over every run of the config
        for arm in ("stock", "pr2830"):
            runs = [r for r in rows if r["config"] == config and r["arm"] == arm
                    and (r["map"], r["start"], config) not in dead]
            if not runs:
                continue
            jumps = [r["goal_jump_total_m"] for r in runs]
            swings = [r["cross_map_swings"] for r in runs]
            lines.append(f"  ping-pong, {arm:7s}: {len(runs):2d} runs  "
                         f"saut but->but cumule med {statistics.median(jumps):6.2f} m "
                         f"total {sum(jumps):7.1f} m  |  traversees de carte "
                         f"total {sum(swings):3d}  med {statistics.median(swings):.1f}  "
                         f"runs avec au moins une: "
                         f"{sum(1 for v in swings if v):2d}/{len(swings)}")

    text = "\n".join(lines)
    print(text)
    with open(os.path.join(HERE, f"agregats{suffix}.txt"), "w") as fh:
        fh.write(text + "\n")
    print(f"\n-> {os.path.join(HERE, f'agregats{suffix}.txt')}")
    return 0
